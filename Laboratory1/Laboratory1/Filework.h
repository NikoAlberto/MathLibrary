...
Matrix& ReadTextFile(Matrix& object, string filet) {
	...
	ifstream file(filet); // файл из которого читаем
	if (!file) {
		// то выводим следующую ошибку и выполняем функцию exit()
		cerr << "Could not be opened for reading!" << endl;//вывод в случае ошибки
		exit(1);
	}
	size_t array_size = 0;
...
//Вывод в текстовый файл перегрузка
...
//Считывание файла в матрицу перегрузка
