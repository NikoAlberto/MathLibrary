...
Matrix::Matrix(int m, int n, vector<double> mat) {
	this->M = m;
	this->N = n;
...
int Matrix::get_Mrows() { return M; }
int Matrix::get_Ncols() { return N; }
void Matrix::set_elm(size_t i, size_t j, double a) { this->matrix[i][j] = a; }
...
Symmetric::Symmetric(vector<double> sym)
{
	if (sym.size() <= 0)
		throw MyException5();
	...
	matrix.resize(M);//ѕолучение размерности матрицы из длины вектора
	for (size_t i = 0; i < M; i++)
	{
		matrix[i].resize(N);
	}
...
UpTriangular::UpTriangular(vector<double> Uptr)
{
	if (Uptr.size() <= 0)
		throw MyException5();
...
LowTriangular::LowTriangular(vector<double> Uptr)
{
	if (Uptr.size() <= 0)
		throw MyException5();
	if (abs(int(sqrt(1 + 8 * Uptr.size())) - sqrt(1 + 8 * Uptr.size())) > pow(10, -5))
	...
	M = (-1 + sqrt(1 + 8 * Uptr.size())) / 2;
	N = (-1 + sqrt(1 + 8 * Uptr.size())) / 2;
	matrix.resize(M);//Getting the dimension of matrix from the length of vector
	for (size_t i = 0; i < M; i++)
	{
		matrix[i].resize(N);
	}
...
Diagonal::Diagonal(vector<double> diagon) {
	if (diagon.size() <= 0)
		throw MyException5();
	M = diagon.size();
	N = diagon.size();
